...
Function New-WMIClass {
	...
	$newClass.Properties["FullName"].Qualifiers.Add("key", $true)
	$newClass.Properties["FullName"].Qualifiers.Add("read", $true)
...
#>
	...
	foreach ($LocalAdministrator in $LocalAdministrators) {
        ...
        $searcherFullName = $LocalAdministrator.User
        <##    
        $adsi = [ADSI]"LDAP://vntad0011.kvv.se"
        $searcher = New-Object DirectoryServices.DirectorySearcher($adsi)
        $searcher.Filter = "(sAMAccountName=$searcherFullName)"
